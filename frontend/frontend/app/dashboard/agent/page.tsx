...
async function publishToYouTube(params: {
  ...
  async function handleStart() {
    ...
    try {
      ...
      if (autoPublish) {
        ...
        // Agar autoPublish nahi tha, toh SEO generate karein
      ...
      // Yeh outer try block ka catch hai jo pure handleStart ko cover karta hai
      setVideoProgress('');
      setErrorMsg(err.message || 'Something went wrong');
      setStages((prev) => prev.map((s) => (s.status === 'working' ? { ...s, status: 'failed' } : s)));
    } finally {
      // Yeh outer try block ka finally hai
      setIsRunning(false);
  ...
  async function handleSchedule() {
    ...
    try {
      const videoPath = currentJobId ? await getVideoPathForJob(currentJobId) : null;
      const videoBase64 = videoPath ? undefined : await videoUrlToBase64(resultVideo);
